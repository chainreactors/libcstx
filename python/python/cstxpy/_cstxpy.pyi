...
class Graph:
    """Graph namespace kept separate from the repository lifecycle."""
    ...
    def nodes_by_ids(self, node_ids: list[str]) -> list[dict[str, Any]]:
        """Materialize selected nodes in caller order."""
        ...

    def link_nodes(
        self, node_ids: list[str], data_source: str
    ) -> dict[str, Any]:
    ...
    def update_node_flags(
        ...
        """Update one node's native flag bitset."""
        ...
    ...
    def subgraph_ids(
        self, seed_ids: list[str], depth: int = 0
    ) -> tuple[list[str], list[str]]:
        """Return node and edge IDs in a seed neighborhood."""
        ...

    def query_node_ids(
        self, expression: str, limit: int | None = None, offset: int = 0
    ) -> list[str]:
        """Execute a query and return terminal node IDs."""
        ...

    def query_trace_ids(
        self, expression: str, limit: int | None = None, offset: int = 0
    ) -> tuple[list[str], list[str], list[str]]:
        """Execute a query and return terminal plus trace IDs."""
        ...

    def induced_snapshot(self, node_ids: list[str]) -> dict[str, Any]:
        """Return a canonical induced-subgraph snapshot."""
        ...
    ...
    def filter_snapshot(
        ...
        """Return a filtered snapshot and exclusion reasons."""
        ...
    ...
    def elevate_snapshot(self, concept_name: str) -> dict[str, Any]:
        """Return an elevated graph snapshot and anchor count."""
        ...

    def add_node(self, node: dict[str, Any]) -> int:
        """Add or merge one canonical native dictionary."""
        ...
    ...
    def add_nodes_json(self, data: bytes) -> int:
        """Single-parse fast path for producers that already own a JSON node array.

        Native Python dictionaries should use :meth:`add_nodes`; encoding them
        first would add serialization work instead of removing it.
        """
        ...

    def hydrate_cas_nodes(self, items: list[tuple[str, str, bytes]]) -> int:
        """Hydrate trusted CAS node bytes without Python JSON decoding."""
        ...

    def add_edge(self, edge: dict[str, Any]) -> int:
        """Add or merge one canonical relationship dictionary."""
        ...
    ...
    def add_edges_json(self, data: bytes) -> int:
        """Single-parse fast path for an existing JSON relationship array.

        Native Python dictionaries should use :meth:`add_edges`.
        """
        ...

    def hydrate_cas_edges(self, items: list[tuple[str, str, bytes]]) -> int:
        """Hydrate trusted CAS edge bytes without Python JSON decoding."""
        ...

    def ingest(self, source: str, data: bytes) -> int:
        """Ingest one linked native-plugin payload into the shared graph."""
        ...
    ...
    def find_node(self, identifier: str) -> dict[str, Any] | None:
        """Resolve by canonical ID, value, then ``extras.name``."""
        ...

    def patch_node_extras(self, node_ids: list[str], patch: dict[str, Any]) -> int:
        """Merge contextual fields into selected nodes in native storage."""
        ...
    ...
    def union_snapshot(self, other: Graph) -> dict[str, Any]:
        """Return the canonical native union with another graph."""
        ...

    def difference_snapshot(
        self, other: Graph, node_type: str | None = None
    ) -> dict[str, Any]:
    ...
    def stats(self) -> dict[str, dict[str, int]]:
        """Return small aggregate counts as a native mapping."""
        ...
    ...
    def nodes_json(self, node_type: str | None = None) -> bytes:
        """Serialize nodes directly in Rust for JSON transport/export workloads.

        Callers that need Python dictionaries should use :meth:`nodes` so they
        do not immediately decode these bytes again.
        """
        ...
    ...
    def edges_json(
        ...
        """Serialize matching edges without constructing Python dictionaries."""
        ...

    def neighbors_json(self, node_id: str, direction: str = "out") -> bytes:
        """Serialize neighboring nodes directly for JSON transport/export."""
        ...
    ...
    def query_json(self, expression: str, limit: int | None = None) -> bytes:
        """Run the same DSL as :meth:`query` and encode matches directly in Rust."""
        ...
...
class Rag:
    ...
    def index(self, request: dict[str, Any]) -> dict[str, Any]:
        """Project graph changes into a deterministic recall delta."""
        ...
...
class CSTX:
    ...
    def graph(self) -> Graph:
        """Return the lightweight graph namespace for this runtime."""
        ...
