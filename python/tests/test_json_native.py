...
def test_json_batch_and_direct_nodes_match_native_cursor():
    ...
    assert value.graph.add_nodes_json(json.dumps(nodes).encode()) == 2
    assert json.loads(value.graph.nodes_json()) == list(value.graph.nodes())
...
def test_json_edges_query_neighbors_and_snapshot():
    ...
    assert value.graph.add_edges_json(json.dumps([relation]).encode()) == 1
    assert json.loads(value.graph.edges_json()) == list(value.graph.edges())
    assert json.loads(value.graph.neighbors_json("ip:1.1.1.1")) == list(value.graph.neighbors("ip:1.1.1.1"))
    assert json.loads(value.graph.query_json("ip")) == list(value.graph.query("ip"))
    snapshot = value.repo.dump_json()
    restored = db()
    restored.repo.load_json(snapshot)
    assert json.loads(restored.graph.nodes_json()) == json.loads(value.graph.nodes_json())
...
def test_invalid_batch_is_atomic():
    ...
    with pytest.raises(CSTXError):
        value.graph.add_nodes_json(json.dumps(bad).encode())
